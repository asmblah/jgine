/*
 *  jGine WebGLRendererMaterial class
 */
define([
    "js/util",
    "../../../Promise",
    ...
    function WebGLRendererMaterial(webGL, material) {
        util.assertConstructor(this, WebGLRendererMaterial);

        RendererMaterial.call(this);
    ...
    WebGLRendererMaterial.prototype = Object.create(RendererMaterial.prototype);

    util.extend(WebGLRendererMaterial.prototype, {
        load: function () {
    ...
    function passTexturesToGL(gl, rendererMaterial) {
        var i,
            textures = rendererMaterial.textures,
            textureUniforms = rendererMaterial.textureUniforms;
    ...
    function locateAttributesForGL(gl, rendererMaterial) {
        var vertexPositionAttribute = gl.getAttribLocation(rendererMaterial.program, "aVertexPosition"),
            uvAttribute = gl.getAttribLocation(rendererMaterial.program, "aTextureCoord");

        gl.enableVertexAttribArray(vertexPositionAttribute);
        gl.enableVertexAttribArray(uvAttribute);

        rendererMaterial.vertexPositionAttribute = vertexPositionAttribute;
        rendererMaterial.uvAttribute = uvAttribute;
    }

    function passFacesToGL(gl, rendererMaterial, faces, verticesBuffer, verticesArray) {
        ...
        gl.vertexAttribPointer(rendererMaterial.vertexPositionAttribute, 3, gl.FLOAT, false, 8*4, 0*4); // Position
        //gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 8*4, 3*4); // Normal
        gl.vertexAttribPointer(rendererMaterial.uvAttribute, 2, gl.FLOAT, false, 8*4, 6*4); // UV
